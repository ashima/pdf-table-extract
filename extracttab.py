...
def procargs() :
  ...
  p.add_argument("-p", dest='page', type=int, required=True,
     help="the page in the PDF to process." )
...
elif args.t == "cells_json" :
  json.dump(cells, args.outfile)

elif args.t == "cells_xml" :
  doc = getDOMImplementation().createDocument(None,"table", None)
  root = doc.documentElement;
  for cl in cells :
    x = doc.createElement("cell")
    map(lambda(a): x.setAttribute(*a), zip("xywh",map(str,cl)))
    if cl[4] != "" :
      x.appendChild( doc.createTextNode(cl[4]) )
    root.appendChild(x)
  args.outfile.write( doc.toprettyxml() )

elif args.t == "table_csv":
  tab = [ [ "" for x in range(len(vd)/2 +1)] for x in range(len(hd)/2+1) ]
  for (i,j,u,v,value) in cells :
    tab[j][i] = value
  csv.writer( args.outfile , dialect='excel' ).writerows(tab)
